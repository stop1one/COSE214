...
// My custom Function
void separate_lpoints( t_point *points, int num_point, t_point from, t_point to, t_point *lpoints, int *num_lpoint);
...
void separate_points( t_point *points, int num_point, t_point from, t_point to, t_point *s1, t_point *s2, int *n1, int *n2){
	float a, b, c, sep;
	a = (float)to.y - (float)from.y;
	b = (float)from.x - (float)to.x;
	c = (float)from.x*(float)to.y - (float)from.y*(float)to.x;
	for (int i = 0; i < num_point; i++){
		if (points[i].x==from.x && points[i].y==from.y) continue;
		if (points[i].x==to.x && points[i].y==to.y) continue;

		sep = a*points[i].x + b*points[i].y - c;
		if (sep < 0) {
			s1[*n1] = points[i];
			*n1 += 1;
		}
...
t_line *upper_hull( t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity){
	//General Case
	if (num_point <= 0){
		//If needed, increase the capacity of lines
	...
	//Find the pmax
	float a = (float)pn.y - (float)p1.y;
	float b = (float)p1.x - (float)pn.x;
	float c = (float)p1.x*(float)pn.y - (float)p1.y*(float)pn.x;
	int maxi = 0; float d, maxd=0;
	for (int i = 0; i<num_point; i++){
	...
	t_point pmax = points[maxi];

	int num_lpoint = 0;
	t_point *lpoints = (t_point *)malloc(sizeof(t_point)*num_point);

	//p1->pmax
	separate_lpoints(points, num_point, p1, pmax, lpoints, &num_lpoint);
	lines = upper_hull(lpoints, num_lpoint, p1, pmax, lines, num_line, capacity);
	//pmax->pn
	num_lpoint = 0; //reset
	separate_lpoints(points, num_point, pmax, pn, lpoints, &num_lpoint);
	lines = upper_hull(lpoints, num_lpoint, pmax, pn, lines, num_line, capacity);
